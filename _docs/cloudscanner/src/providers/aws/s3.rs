...
pub trait S3Discovery: Send + Sync {
    async fn discover_buckets(&self) -> Result<Vec<Asset>, CloudScannerError>;
}
...
impl S3Discovery for MockS3Discovery {
    async fn discover_buckets(&self) -> Result<Vec<Asset>, CloudScannerError> {
        ...
        Ok(vec![
            Asset::new("s3-bucket".to_string(), "mock-bucket-1".to_string())
                .with_metadata("region".to_string(), "us-east-1".to_string())
                .with_metadata("public".to_string(), "true".to_string()),
            Asset::new("s3-object".to_string(), "mock-bucket-1:mock-file.txt".to_string())
