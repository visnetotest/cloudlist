...
| Goal ID | Title | Description | Related TR | Status |
| :--- | :--- | :--- | :--- | :--- |
| **G-1** | **AWS S3 Provider Plugin** | Implement a provider plugin for AWS that can discover S3 buckets, as detailed in TRD-017. | TR-1, TR-2, TR-17 | **Design Phase**. The technical approach is defined in `TRD-017-AWS-S3-Provider-Plugin.md`. Next steps are to implement the Rust crate and integrate it with the `cloudscanner` engine. |
| **G-2** | **YAML Policy for Public S3** | Implement a policy in the core engine that can identify S3 buckets that are publicly accessible, as detailed in TRD-018. | TR-4, TR-18 | **Design Phase**. The technical approach is defined in `TRD-018-YAML-Policy-for-Public-S3.md`. Next steps are to implement the policy engine in Rust and update the S3 provider to add the necessary metadata. |
| **G-3** | **End-to-End Scan** | Demonstrate a complete, end-to-end scan using the unified engine: `discover` (AWS S3) -> `evaluate` (public S3 policy) -> `report` (JSON output). | TR-3, TR-5 | **Not Started**. The `cloudscanner` engine is still under development and not yet unified. The `cloudlist` tool performs discovery, but the policy evaluation and reporting are separate. Configuration is handled by a separate `provider-config.yaml`, not a unified `cloudscanner.toml`. |
| **G-4** | **Live Event Detection** | Demonstrate that creating a new, public S3 bucket in a test AWS account triggers an immediate, event-driven scan that flags the new resource within 60 seconds. | TR-6 | **Design Phase**. `TRD-009-Event-Driven-Discovery.md` outlines the design for this feature, but no implementation is present in the codebase. |
| **G-5** | **Attack Path Visualization** | Demonstrate the construction of an asset graph from a test environment and export it to a DOT file. The graph must show a connection from an EC2 instance to an assumed IAM role to an S3 bucket. | TR-7 | **Design Phase**. `TRD-010-Asset-Graph.md` describes the architecture for an asset graph, but the implementation is not yet started. The current tool only outputs lists of resources. |
| **G-6** | **CI/CD Quality Gate** | Implement an automated end-to-end test in the CI/CD pipeline that fails the build if a known, critical misconfiguration is not detected. | TR-8 | **Partially Implemented**. The project has CI/CD pipelines for linting (`lint-test.yml`) and building (`build-test.yml`). However, there are no end-to-end tests that validate the detection of misconfigurations. The `_docs/poc2/test/` directory contains setup for a test environment, which is a good first step. |

## 5. Implementation and Testing Strategy

This section outlines the practical steps for implementing and testing the PoC goals. Use "gresau/localstack-persist:latest examples" for localstack testing as it has persistences.
